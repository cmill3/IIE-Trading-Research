...
def tda_PUT_3D_stdcls(polygon_df, simulation_date, quantity, config, target_pct, vol, standard_risk):
    ...
    for index, row in polygon_df.iterrows():
        ...
        if min_value < (2*target_pct):
            Floor_pct = (.95*underlying_gain)
        elif min_value < target_pct:
...
def tda_CALL_3D_stdcls(polygon_df, simulation_date, quantity, config, target_pct, vol, standard_risk):
    open_price = polygon_df.iloc[0]['underlying_price']
    derivative_open_price = polygon_df.iloc[0]['o']
    for index, row in polygon_df.iterrows():
        if index == 0:
        ...
        if max_value > (2*target_pct):
            Floor_pct = (.95*underlying_gain)
        elif max_value > target_pct:
            Floor_pct = (.8*underlying_gain)
...
def tda_PUT_1D_stdcls(polygon_df, simulation_date, quantity, config, target_pct, vol, standard_risk):
    open_price = polygon_df.iloc[0]['underlying_price']
    derivative_open_price = polygon_df.iloc[0]['o']
    for index, row in polygon_df.iterrows():
        if index == 0:
...
def tda_CALL_1D_stdcls(polygon_df, simulation_date, quantity, config, target_pct, vol, standard_risk):
    open_price = polygon_df.iloc[0]['underlying_price']
    derivative_open_price = polygon_df.iloc[0]['o']
    for index, row in polygon_df.iterrows():
        if index == 0:
