...
def tda_PUT_3D_RMF(polygon_df, simulation_date, quantity, config, target_pct, vol, standard_risk):
...
def tda_CALL_3D_RMF(polygon_df, simulation_date, quantity, config, target_pct, vol, standard_risk):
    open_price = polygon_df.iloc[0]['underlying_price']
    derivative_open_price = polygon_df.iloc[0]['o']
    target_pct = float(target_pct)
    for index, row in polygon_df.iterrows():
...
def tda_PUT_1D_RMF(polygon_df, simulation_date, quantity, config, target_pct, vol, standard_risk):
    open_price = polygon_df.iloc[0]['underlying_price']
    target_pct = float(target_pct)
    derivative_open_price = polygon_df.iloc[0]['o']
    for index, row in polygon_df.iterrows():
...
def tda_CALL_1D_RMF(polygon_df, simulation_date, quantity, config, target_pct, vol, standard_risk):
    open_price = polygon_df.iloc[0]['underlying_price']
    target_pct = float(target_pct)
    derivative_open_price = polygon_df.iloc[0]['o']
    for index, row in polygon_df.iterrows():
