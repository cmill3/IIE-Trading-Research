...
def tda_PUT3D_CDVOLVARVC(polygon_df, simulation_date, quantity, config, target_pct, vol, order_num,symbol):
...
def tda_CALL3D_CDVOLVARVC(polygon_df, simulation_date, quantity, config, target_pct, vol, order_num,symbol):
    open_price = polygon_df.iloc[0]['underlying_price']
    derivative_open_price = polygon_df.iloc[0]['o']
    Floor_pct = (-vol * config['volatility_threshold'])
    if order_num > 4:
...
def tda_PUT3D_CDVOLSTEP(polygon_df, simulation_date, quantity, config, target_pct, vol, order_num,symbol):
    open_price = polygon_df.iloc[0]['underlying_price']
    derivative_open_price = polygon_df.iloc[0]['o']
    Floor_pct = (-vol * config['volatility_threshold'])
    if order_num > 4:
    ...
    for index, row in polygon_df.iterrows():
        ...
        if day_diff < 3:
            ...
            if day_diff == 1:
                ...
                Floor_pct = Floor_pct * float(step_values[1])
...
def tda_CALL3D_CDVOLSTEP(polygon_df, simulation_date, quantity, config, target_pct, vol, order_num,symbol):
    open_price = polygon_df.iloc[0]['underlying_price']
    derivative_open_price = polygon_df.iloc[0]['o']
    Floor_pct = (-vol * config['volatility_threshold'])
    if order_num > 4:
