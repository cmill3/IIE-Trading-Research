...
def run_trades_simulation(full_positions_list,start_date,end_date,config,period_cash):
    ...
    return portfolio_df, positions_df, cash_reserve
...
def backtest_orchestrator(start_date,end_date,file_names,strategies,local_data,config,period_cash):
    ...
    portfolio_df, positions_df, cash_reserve = run_trades_simulation(full_df, start_date, end_date, config, period_cash)
    return portfolio_df, positions_df, full_df, cash_reserve
...
if __name__ == "__main__":
    ...
    for config in backtest_configs:
        for year in years:
            ...
            trading_strat = f"{config['user']}/{nowstr}-{year_data['year']}:{config['aa']}_{config['dataset']}_{config['days']}_{config['holiday_weeks']}_{config['portfolio_pct']}_{config['model']}_CD{config['capital_distributions']}_vol{config['volatility_threshold']}_vc{config['vc_level']}_sssl{config['spread_search']}:{config['spread_length']}"
